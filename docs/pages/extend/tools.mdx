...
- `type = "hmac_sign"` signs outbound requests inside iron-proxy; the signing
  key never enters the sandbox. api-rs validates this metadata for discovery
  but does not register an HMAC signing transform. An operator must register
  the declared secret and grant it to only the intended tool/principal with
  `centaur-perms` before the tool can make signed requests. Set `hosts`,
  `http_methods`, and `paths` to the narrowest allowed requests; for an
  exact webhook use a single `POST` method and its absolute path. Invalid
  method/path restrictions are rejected rather than ignored. A visible tool
  without its HMAC grant still cannot produce an accepted signature.
