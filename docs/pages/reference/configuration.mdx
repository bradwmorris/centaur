...
| `CODEX_AUTH_MODE` | `sandbox.codexAuthMode`. | Codex auth flow: `access_token` (chart default, hits `chatgpt.com` via the brokered ChatGPT login) or `api_key` (rollback, hits `api.openai.com`). The chart sets this on api-rs, which propagates it into sandboxes. See [Codex Auth Modes](/deploying-in-production#codex-auth-modes). |
