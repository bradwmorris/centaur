...
| `CENTAUR_CONTEXT_API_KEY` | Context Curator to private inference API | Dedicated static bearer token with only the Curator inference capability. Required when `apiRs.curatorInference.enabled=true`; keep it distinct from every other API key. |
...
|------|----------|----------------------|
| `access_token` (chart default) | `chatgpt.com` | `OPENAI_CODEX_ACCOUNT_ID` in the secrets backend, plus the `openai-codex` broker credential in the console |
| `api_key` (rollback) | `api.openai.com` | `OPENAI_API_KEY` in the secrets backend |
...
To run Centaur Context's Curator on the same subscription, set
`apiRs.curatorInference.enabled=true`, keep `sandbox.codexAuthMode=access_token`,
and configure Context with the private api-rs endpoint, the matching
`CENTAUR_CONTEXT_API_KEY`, and exact model `gpt-5.6-luna`. Each request runs in
an ephemeral repo-less sandbox with read-only Codex execution and a structured
output schema; api-rs rejects observed command or MCP tool activity and does
not create a durable Centaur session. Set Context's transport to `direct_api`
to roll back without changing the agent authentication mode.

For a complete API-key rollback, change both chart values together:
...
Then set Context's `CURATOR_MODEL_TRANSPORT=direct_api` and point its model URL,
token, and model at the approved direct provider before restarting either
deployment. Existing in-flight subscription turns may fail during the rollout;
they are not replayed across authentication modes, while queued Curator runs
remain retryable under their stable run/attempt idempotency keys.
